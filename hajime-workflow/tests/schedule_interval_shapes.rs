...
//!
//! `hajime-workflow --validate` run against the real backup
//! (`carbonflow-20260806-155116/db/n8n_workflows.json`, 17 workflows, 5
//! active) passed clean: every node kind in the file has an executor and no
//! schedule was rejected. But three of the four active schedules use
//! `cronExpression` directly, and `triggers.rs` only unit-tested the
//! `minutes` and `days` friendly forms of the interval field -- never
//! `hours`. The one active workflow that uses it, "CarbonFlow Node B - SEO
//! and Content Optimization (RAG)", was validated only by the flag's exit
//! code, not by a test that would fail loudly if the conversion broke.
//!
