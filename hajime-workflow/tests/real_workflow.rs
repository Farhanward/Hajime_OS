//! Runs a workflow exported from the live n8n instance, unmodified.
//!
//! The fixture is the real `🚨 CarbonFlow — Hourly Health Monitor`, taken from
//! the backup. It exercises four node kinds at once and, more usefully, it
//! catches the two things unit tests could not: parameters that are expressions
//! evaluated per item, and nodes that must run once per item rather than once
//! per list.
//!
...
fn the_export_loads_without_conversion() {
    let wf = workflow();
    assert_eq!(wf.name, "🚨 CarbonFlow — Hourly Health Monitor");
    assert!(wf.active);
...
#[tokio::test]
#[ignore = "makes real requests to carbonflows.store"]
